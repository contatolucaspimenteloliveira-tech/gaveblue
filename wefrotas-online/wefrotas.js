let allVehicles = [];
    ...
    async function handleCustomLogoUpload(event) {
      ...
      if (window.WeFrotasBackend?.getUser()) {
        try {
          ...
          customLogoUrl = await window.WeFrotasBackend.uploadReceipt(file);
          customLogoEnabled = true;
          updateCustomLogoUi();
          showToast('Logo enviada. Agora clique em salvar para aplicar.');
        } catch (error) {
          showToast(error?.message || 'Não foi possível enviar a logo.');
        } finally {
          event.target.value = '';
        }
        return;
    ...
    function updateFinanceReceiptPreview(url = '') {
      ...
      if (normalizedUrl) {
        ...
        wrapper.classList.remove('hidden');
        labelNode.textContent = 'Nenhum comprovante anexado';
        actionButton.disabled = true;
      }
    }

    async function handleFinanceReceiptUpload(event) {
      const input = event?.target;
      const file = input?.files?.[0];
      if (!file) return;
      try {
        showToast('Enviando comprovante...');
        const url = await window.WeFrotasBackend?.uploadReceipt(file);
        if (!url) throw new Error('O backend não retornou o endereço do comprovante.');
        updateFinanceReceiptPreview(url);
        showToast('Comprovante enviado com sucesso.');
      } catch (error) {
        showToast(error?.message || 'Não foi possível enviar o comprovante.');
      } finally {
        if (input) input.value = '';
      }
    }

    window.handleFinanceReceiptUpload = handleFinanceReceiptUpload;
    ...
    function loadFinanceForm(entryType) {
      ...
      if (entryType === 'combustivel') {
        fields.innerHTML = `
          ...
          <div class="field-wrap full" id="finance-receipt-wrap">
            <label>Comprovante</label>
            <input id="finance-comprovante-url" type="hidden">
            <div class="soft-input w-full flex items-center justify-between gap-3">
              <span id="finance-receipt-url" class="truncate text-slate-500">Nenhum comprovante anexado</span>
            ...
            <div class="online-receipt-upload">
              <input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onchange="handleFinanceReceiptUpload(event)">
              <span class="text-xs text-slate-500">Imagem ou PDF de até 20 MB.</span>
            </div>
          </div>
          <div class="field-wrap full">
            <label>Observações</label>
