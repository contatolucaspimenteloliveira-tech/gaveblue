  ...
  return `
    ...
    <main class="screen admin">
      ...
      <div class="admin-tabs">
        ${["dashboard", "estabelecimentos", "eventos", "categorias", "avaliacoes"].map((item) => `<button class="${tab === item ? "selected" : ""}" data-admin-tab="${item}">${item}</button>`).join("")}
      </div>
      ${tab === "dashboard" ? dashboard(data) : ""}
      ${tab === "estabelecimentos" ? establishmentsAdmin(data) : ""}
      ${tab === "eventos" ? eventsAdmin(data) : ""}
