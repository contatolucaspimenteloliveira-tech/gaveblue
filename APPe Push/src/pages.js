...
import { bindCommon, bottomNav, categoryById, categoryGrid, emptyState, eventCard, miniPlaceCard, placeCard, qs, qsa, searchBox, sectionTitle, statusPill, topBar } from "./components.js";
...
export function explorePage(params = {}) {
  ...
  if (filter === "rating") places.sort((a, b) => b.rating - a.rating);
  ...
  return `
    ...
    <main class="detail-screen">
      ...
      <section class="detail-card">
        ...
        <div class="rating-line"><span>★ ${place.rating.toFixed(1)}</span><span>${place.reviewCount + reviews.length} avaliações</span></div>
        ${statusPill(status)}
...
export function eventsPage(params = {}) {
  ...
  const featured = data.events.find((event) => event.featured) || events[0];
  return shell(`
    ...
    <div class="list">${events.length ? events.map((event) => eventCard(event)).join("") : emptyState("Nenhum evento encontrado", "Tente outro filtro.")}</div>
  `, "eventos", topBar());
}
