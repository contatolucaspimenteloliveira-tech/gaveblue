const defaultConfig = {
  page_title: 'Central de Registros',
...
const DRIVER_NAMES_STORAGE_KEY = 'central-de-registros:driver-names';
const LAST_FUEL_ENTRY_STORAGE_KEY = 'central-de-registros:last-fuel-entry';
...
function openLooseNoteForm() {
  const modal = document.getElementById('loose-note-modal');
...
function validateFuelReceiptUploadFields(formData) {
  if (!formData.motorista || !formData.cidade || !formData.posto || !formData.data || !formData.valor || !formData.litros || !formData.tipoCombustivel) {
    showErrorMessage('Preencha motorista, cidade, posto, data, valor, litros e combust\u00edvel antes de salvar o comprovante.');
    return false;
  }
...
async function submitFuelForm(e) {
  ...
  const mensagem = [
    ...
    `\ud83d\udcc5 *Data/Hora:* ${formData.dataFormatada} \u00e0s ${formData.horaFormatada}`,
    `\ud83d\udcb0 *Valor:* ${formData.valor}`,
    `\ud83d\udee2\ufe0f *Litros:* ${formData.litros}`,
    `\u26fd *Combust\u00edvel:* ${formData.tipoCombustivel}`,
