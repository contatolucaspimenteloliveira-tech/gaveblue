let allVehicles = [];
    ...
    async function applyExternalStorageSnapshot(snapshot = {}) {
    ...
    window.wefrotasLocalData = {
      getSnapshot: buildStorageSnapshot,
      applySnapshot: applyExternalStorageSnapshot,
      saveLocal: saveToLocalStorage,
      refresh: renderAll,
      notify: showToast,
      getStorageEngine: () => wefrotasStorageEngine
    };
    ...
    async function initializeWeFrotas() {
      ...
      updateManagerIdentityUi();
      updateOperationSettingsUi();
      window.dispatchEvent(new CustomEvent('wefrotas:local-ready', {
        detail: { storageEngine: wefrotasStorageEngine }
      }));
    }
